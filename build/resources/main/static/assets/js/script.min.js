...
// 소분류 안에 값 찍어내는 공장
        ...
        function savePageSize(pageSize) {
            ...
            switch (boardTitle.innerHTML) {
                case "게시판 관리":
                    sortUrl = "/fooditem"
                    thisData = {"tab":a, "pageNum":pageNum, "pageSize":pageSize}
                    break;
                case "재고 관리":
                    sortUrl = "/inventory/list"
                    thisData = {"pageNum": pageNum, "pageSize": pageSize, "currentMenu": boardTitle.innerHTML.substring(0,1)}
                    break;
                default:
        ...
        function pageNumChange(e) {
            ...
            $.ajax({
                method: 'get',
                url: "/boardlist",
                data: {"tab": tab, "pageNum": pageNum, "pageSize": pageSize}
            }).done((resp) => {
                console.log("페이지이동 DONE")
                sortClassSwitch(currentID, null, resp)
            }).fail((err) => {
        ...
        function sortClassSwitch2(id, className, resp) {
            ...
            if (id.substring(0, 1) === '재') {
                ...
                str += "<th>#</th>"
                str += "<th><h4 style='text-align: center'>등록일<a onclick='boardListSortButton(this.id)' id='fcode' class='" + f_date + "'><img src='/assets/img/a/sort.png' style='width: 20px; height: 20px'></a></h4></th>";
                ...
                str += "<th><h4>카테고리<a onclick='boardListSortButton(this.id)' id='fcnum' class='" + f_cname + "'><img src='/assets/img/a/sort.png' style='width: 20px; height: 20px'></a></h4></th>"
                str += "<th><h4>상품코드<a onclick='boardListSortButton(this.id)' id='fcnum2' class='" + f_code + "'><img src='/assets/img/a/sort.png' style='width: 20px; height: 20px'></a></h4></th>"
                ...
                str += "<th><h4>수량<a onclick='boardListSortButton(this.id)' id='fprice' class='" + f_count + "'><img src='/assets/img/a/sort.png' style='width: 20px; height: 20px'></a></h4></th>"
                str += "<th><h4>평균금액<a onclick='boardListSortButton(this.id)' id='fcount' class='" + f_avgPrice + "'><img src='/assets/img/a/sort.png' style='width: 20px; height: 20px'></a></h4></th>"
                str += "<th><h4>총 계<a onclick='boardListSortButton(this.id)' id='fdate' class='" + f_countSum + "'><img src='/assets/img/a/sort.png' style='width: 20px; height: 20px'></a></h4></th>"
...
// 게시글 정렬 버튼
        ...
        function boardListSortButton2(id) {
            let ID = document.getElementById(id);
            if (ID.className === "Asort selected") {
                ID.classList.remove("selected");
            } else if (ID.className === "Asort") {
...
// 자바스크립트 폼데이터 객체 관련
        {
            function modalInputForm() {
                ...
                $.ajax({
                    type: 'POST',
                    url: "/fooditem/modalinput?" + "pageNum=" + pageNum + "&pageSize=" + pageSize,
                    data: formData,
                    contentType: false,
                    processData: false
                }).done((resp) => {
