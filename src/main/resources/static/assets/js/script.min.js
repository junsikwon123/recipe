...
// 대분류 카테고리 추가 함수
    function bigCgAdd() {
        ...
        $.ajax({
            method: 'get',
            url: "/addbigcg?bigCgName=" + bigCgName + "&cg=" + cg
        }).done((resp) => {
            console.log("추가된 대분류 다시 뿌리기")
            let bigCgDiv = $('#bDiv')
            let printInDiv = "";
            let i = 1; // div의 인덱스
            let j = 0; // 버튼의 인덱스
...
// 카테고리 통합 삭제 로직 [5]
{
    function deleteThis(cgName, element, num) {
        ...
        let printInDiv;
        let cg= "";
        console.log("할아버지 : " + grandParent);
        console.log("아빠 : " + parent)
        if (grandParent === 'bDiv') {
            printInDiv = $('#bDiv')
            cg = 'b';
...
// 자바스크립트 폼데이터 객체 관련
{
    function modalInputForm() {
        ...
        let fiFile = document.querySelector('#preview').files[0].name
